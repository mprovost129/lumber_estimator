...
class SplicedFormulaTests(TestCase):
    """per_length_spliced and per_area_spacing_spliced: members longer than the
    longest stock piece are built by splicing full-length pieces end to end,
    instead of raising 'no stock length covers X'."""

    def setUp(self):
        # Stock tops out at 20 ft, so anything longer must be spliced.
        self.material = MaterialProduct.objects.create(
            name='Spliced Beam Stock', input_type=MaterialProduct.InputType.FT,
        )
    ...
    def _rule(self, tool_type='line', **kwargs):
        assembly = Assembly.objects.create(name='Spliced Assembly', tool_type=tool_type)
        defaults = {'assembly': assembly, 'material': self.material, 'role': 'Beam Ply'}
        defaults.update(kwargs)
        return CalculationRule.objects.create(**defaults)
    ...
    def test_per_area_spacing_spliced_splices_each_member(self):
        rule = self._rule(
            tool_type='area', formula_kind=CalculationRule.FormulaKind.PER_AREA_SPACING_SPLICED,
        )
        # 16 ft run of members at 16" OC = ceil(192/16)+1 = 13 members.
        # Each member is 30 ft long -> ceil(30/20) = 2 pieces. 13 x 2 = 26.
        measurement = {
    ...
    def test_over_length_still_raises_for_non_spliced_per_length(self):
        ...
        rule = self._rule(formula_kind=CalculationRule.FormulaKind.PER_LENGTH, multiplier=1)
        with self.assertRaises(ValueError):
            evaluate_rule(rule, {'length_ft': Decimal('44')}, {})

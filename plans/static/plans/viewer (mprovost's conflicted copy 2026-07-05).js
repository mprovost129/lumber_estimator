...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var defaultWallHeight = parseInt(root.dataset.defaultWallHeight, 10) || 108;
    ...
    function configureToolPanel(tool, variantLabel, settingsFieldsOverride) {
        ...
        populateAssemblyOptions(assemblySelect, tool, collectSettings(tool, ''), activeVariantFilter);
        populatePresetOptions(tool);
        colorInput.value = config.color;
    }

    function populateAssemblyOptions(select, toolType, settings, variantFilter) {
        ...
        if (variantFilter && candidates.length === 1) {
    ...
    document.addEventListener('keydown', function (event) {
        if (event.key === 'Escape') {
            ...
            // segment wall polyline - falling back to a full cancel only
            // when there's nothing yet worth keeping (below the tool's
            // minimum point count).

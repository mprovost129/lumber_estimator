...
9. **Command palette (#17).** Lower risk than the others in this phase, but low ROI until the action surface (right-click menu, bulk tools, shortcuts) from Phase A/B exists to search over - sequence this last in Phase C, or bundle into Phase A if it turns out cheap with an existing library.
10. **Automatic plan recognition (#11).** CV/ML wall-and-opening detection from a rasterized PDF/image. Real value (huge click reduction) but real risk: false positives directly corrupt a material list, which violates "the material list is the product" (Key Business Rules). Needs a spike against real customer plans to establish accuracy before any UI commitment, and should ship as a **suggestion layer the user must confirm per element**, never auto-committed traces.
11. **AI Assistant (#16).** Same caution as #11 - "intelligent framing suggestions" is not yet a scoped feature. Do not start building until there's a concrete, narrow use case (e.g. "flag a wall assembly that's inconsistent with its neighbors") rather than an open-ended assistant.
12. **Sketch Mode (flagship, reframed - see disagreement below).** Re-scope from "freehand sketch -> auto-framed assemblies" to "fast full-building layout on a calibrated page using straight-line/ortho-snap tools that are already built" (endpoint snap, ortho snap, one-click openings). A true uncalibrated freehand-to-BOM pipeline is a separate, much larger CV problem and shouldn't block this rename/rescope.

...
- Every `MaterialProduct` has an `input_type` (`ft` / `box` / `each`) that drives which other fields are relevant - enforced in `MaterialProduct.clean()` and toggled dynamically in the admin form via `catalog/static/catalog/admin/material_product.js`:
  - **Updated**: `input_type` is now the material's **default** ordering method, while `supported_input_types` stores every ordering mode the material can support. This lets one SKU work in more than one estimating context - e.g. a joist can still have stock lengths for cut-to-length/spliced rules while also being valid as an `each` material in piece-count workflows.
  - **ft** support: stock lengths live in `MaterialLength`; `MaterialProduct.stock_length_for(required_ft)` returns the smallest in-stock length that covers a requirement.
  - **box** support: `quantity_per_box` (e.g. 100 screws/box); `MaterialProduct.boxes_needed(quantity)` rounds up to whole boxes.
  - **each** support: no extra fields.
...
- **Library management (added).** The Library page now supports full account-owned material management: users can create a private material manually, edit an existing private material's metadata/input type/stock lengths/default length, or delete it directly from the material card. Stock/global materials remain read-only in the library. Deletes are one click from the library UI but are blocked with a friendly message if the material is still referenced by any `CalculationRule` or `LineItem` (`on_delete=PROTECT` on those FKs).
...
## Marketing site, pricing, brand connectors, import, and the visibility sidebar

**Marketing home + pricing (public).** `core.HomeView` now redirects signed-in users to the dashboard and serves anonymous visitors a full marketing page (`templates/core/home.html`: hero, 3-step how-it-works, feature grid, CTAs, footer; `.mkt-*` styles in `main.css`). `core.PricingView` at `/pricing/` renders `templates/core/pricing.html` driven entirely by the billing settings (`subscription_plans()` + `estimate_unlock_offer()` from `billing.services`), so marketing copy can never drift from what Stripe checkout actually charges. Free / single-unlock / subscription cards; signed-in users get real POST forms to `billing:subscribe` (disabled with a tooltip when a plan has no Stripe price id), anonymous visitors get sign-in CTAs. A Billing link was added to the rail. Tests: `core/test_pages.py`.

**Tier gating.** Output gating (print header context + CSV) was already built in the billing app (`estimate_output_access`: active subscription, else per-estimate paid grant, else locked). This pass wired the missing marketing surfaces to it rather than duplicating any logic.

**Brand connectors seeded.** `catalog/migrations/0011_seed_brand_connectors.py` adds 53 widely stocked Simpson Strong-Tie (LUS/HUS/HGUS/LSSR/HUC/IUS/HU hangers, H-series hurricane ties, A35, LSTA/MST/CS straps, ABU/BC/BCS/AC post hardware, DTT deck ties, HDU hold-downs) and USP (MiTek) (JUS/THD hangers, RT rafter ties, MP34, PA/PC post hardware, DTB) SKUs, deliberately limited to the everyday residential lineup. Brand lives in `species`, series in `grade`, what-it-fits in `nominal_dimension` (20-char limit). Global connectors: 66; global materials: 161.

**Material import.** `estimating.MaterialImportView` at `library/materials/import/` (button in the Library header) accepts CSV or XLSX (openpyxl, added to requirements). Header row required; columns case-insensitive: name (required), category (key or label), species, grade, dimension, input_type (ft/box/each), quantity_per_box, lengths (semicolon/comma feet), default_length. Rows land as account-owned products; names already in the account are skipped so re-imports are safe; per-row problems are reported (capped at 10 messages) while good rows still import inside one transaction; 2 MB / 2000 row limits. Tests: `estimating/test_material_import.py` (CSV, XLSX, skip-existing, bad rows, tenancy, login).

**Visibility sidebar.** The material list panel in the viewer now doubles as the layer/visibility control: every trace-linked row renders a `.mat-vis-toggle` eye button (partial template). In `viewer.js`, hidden state is keyed by material label + category (stable across the partial re-render that follows every trace change) and persisted per page in sessionStorage; `applyMaterialVisibility()` recomputes canvas visibility from scratch each time (a trace linked from several rows, e.g. one wall feeding stud and plate rows, hides while ANY of its rows is hidden), runs after every summary refresh and on initial load. The toggle listener registers before the row-link listener and uses `stopImmediatePropagation` so an eye click never also selects the row. The old "Default visible layers" block was removed from the tool settings sidebar: the block is `hidden` (inputs remain, all checked, because `collectWallSettings` snapshots them into every wall trace) and `wall-layers` was dropped from the wall tool's `settingsFields`; per-wall layer control stays in the wall modal.

**Spec-test fixes shipped with this pass.** Three committed-failing tests now pass: dashboard workflow puts `has_line_items` first so a manual list reads "Estimate ready" even with no plans; the CSV category-order test gets the same paid `EstimateAccessGrant` its sibling tests use; and `_attach_label_lengths()` gives length-less rows (studs traced without a wall height) a display-only label length from the measured run of their linked traces when all traces agree, keeping the order list's length column honest.

Suite: 319 tests + 30 subtests, all passing. Fresh-database migrate seeds 161 materials / 66 connectors / 53 assemblies.

## Drawing assists (fewer clicks per trace)

Four click-savers in the plan viewer, all client-side in `plans/static/plans/viewer.js` plus small template additions. The tool hints teach each one.

- **Endpoint snap.** A click within 12 screen pixels (zoom-adjusted via `snapRadiusCanvasPx()`) of an existing trace endpoint lands exactly on it; the in-progress chain's own first point is also a target so loops close cleanly. Candidates are read live off the canvas objects (`traceGeometry` / `traceToolType` set by `drawTrace`), so deletions and visibility toggles self-heal. This is not just convenience: precise shared endpoints are what make wall junction detection (corner/T stud counts) fire reliably.
- **Ortho snap.** Within 8 degrees of a 45-degree increment, the segment locks to the exact angle from the previous point. Hold Shift for free angles. Both snaps apply to the committed click and the rubber-band preview via one `applyDrawingSnaps(pointer, event)` call in `mouse:down` and `mouse:move`, so what you see is what commits. Snapping tools: line, polyline, opening.
- **Live length readout.** While drawing on a calibrated page, a dark chip follows the cursor showing the current segment as feet-inches (`formatFeetInches`). The numeric scale now reaches the client: `data-scale-pixels-per-foot` on the viewer root (template) and `onCalibrated(data)` captures the scale returned by both calibration flows, so the readout works immediately after calibrating without a reload. Styled by `#draw-measure-readout` in `main.css`; `#plan-canvas-wrap` gained `position: relative` to anchor it.
- **Backspace vertex undo.** While a shape is in progress, Backspace removes the last vertex (and its marker) instead of forcing Escape and a full redraw. Guarded by the existing `isTypingTarget` check and placed before the tool shortcuts in the keydown handler.
- **One-click openings.** With the new Width (in) input (default 36, first field in the opening-size row, snapshotted into trace settings as `rough_width_in` and remembered by tool memory), a single click on or near a wall places a complete opening: projected onto the nearest wall segment (`nearestWallSegment` walks line/polyline canvas objects with `projectPointToSegment`), centered at the click, sized to the width, perfectly aligned so dynamic parent-wall resolution always matches. Falls back to the classic two-click flow when uncalibrated, the width is blank, or the click is not near a wall. The server stores extra settings keys as-is, so no backend change was needed.
- **Duplicate selected trace.** The inspector now exposes a `Duplicate` button, and pressing `D` while a trace is selected does the same thing. The viewer snapshots the selected trace's geometry, material, assembly, parent wall, color, and settings, recreates it through the normal trace-create endpoint, offsets the copy by 24 image pixels down/right, and immediately selects the new copy so repeated duplication is fast. `undoLastDeletedTrace()` now reuses the same snapshot-based create helper, keeping trace recreation behavior consistent in one place.
- **Sample an existing trace into the tool panel.** The inspector now includes `Use for new traces`, which turns the selected trace into the current drawing setup: same material, assembly, color, and settings, with semantic re-arming when the trace clearly maps to a Wall, Beam, Opening, Joist, or Column flow. This avoids the common reconfiguration loop of selecting a trace just to remember how it was built, then manually re-picking the same assembly before drawing the next run.
- **Faster page-to-page movement.** The viewer header now includes previous/next page buttons, and `Page Up` / `Page Down` move between pages from the keyboard. Navigation follows the same ordering as the thumbnail strip, so users tracing a full plan set can keep moving without constantly scrolling back to the page strip first.
- **Cross-page tool handoff.** Moving between pages inside the same project now carries the active drawing setup with you: tool, semantic variant, material, assembly, color, and settings are stashed in sessionStorage on page navigation and restored on the next viewer page. If the destination page is not calibrated yet, the handoff survives through calibration and reapplies as soon as the measuring tool is re-armed.
- **Bulk actions for linked groups.** When a material-list row highlights a linked set of source traces, the toolbar now exposes `Apply tool setup` and `Delete linked`. `Apply tool setup` pushes the currently armed tool panel's material, assembly, color, and settings onto every linked trace whose `traceToolType` matches the active tool, giving a quick batch-reassignment path without a backend batch endpoint. `Delete linked` removes the whole linked group in one action, and the undo banner now restores either one trace or a whole deleted batch.
- **Canvas multi-select feeds the same bulk tools.** Fabric's native active selection is now recognized in `onTraceSelected()`: if the user shift-clicks or box-selects multiple trace objects on the plan, the viewer treats them as a linked working set, updates the summary text, and enables the same batch actions (`Apply tool setup`, `Delete linked`) without requiring the material list row as the entry point.
- **Bulk editor panel for mixed selections.** Multi-selecting several traces now opens a dedicated bulk panel in the right sidebar instead of falling back to an empty state. The panel summarizes the selected trace types, lets the user optionally apply a material tag, assembly (when every selected trace shares the same `traceToolType`), and color across the whole selection, and exposes delete in the same place. This keeps bulk edits discoverable even when the selection did not come from a material-list row.
- **Backend batch endpoints now back the bulk UI.** `plans.views.TraceBatchUpdateView` and `TraceBatchDeleteView` replace the viewer's fan-out loops with one transactional request per bulk action. The batch update contract is intentionally explicit: `trace_ids` plus `apply_material`, `apply_assembly`, `apply_color`, and `apply_settings` flags, so mixed-type selections can safely batch material/color while same-type selections can also batch assemblies/settings. The viewer now posts to `plans:trace-batch-update` / `plans:trace-batch-delete`, and tests live in `plans/test_trace_batches.py`.

Suite unchanged: 319 tests + 30 subtests passing; `node --check` clean.

## Signup, production media storage, and parallel tests

**Self-serve signup.** `accounts.SignUpView` at `/accounts/signup/` (routed ahead of the contrib auth urls) with `accounts.forms.SignUpForm`: company name becomes the tenant `Account`, email/password become its first user, validated by `validate_password` (dev clears the validators, so the weak-password test pins one via `override_settings`). Login must name the backend explicitly (`backend='django.contrib.auth.backends.ModelBackend'`) because axes registers a second one. Every marketing CTA (home, pricing, login page) now routes to signup instead of dead-ending at login. Tests: `accounts/test_signup.py` (tenant creation, per-signup tenant isolation, duplicate email, mismatch, weak password, authed redirect, CTA links).

**Production media storage (Render).** Render web service filesystems are ephemeral, so the previous prod `FileSystemStorage` lost every uploaded plan on deploy. `config/Settings/prod.py` now switches on env: with `AWS_STORAGE_BUCKET_NAME` set it uses `storages.backends.s3.S3Storage` (django-storages[s3] added to requirements; `AWS_S3_ENDPOINT_URL` supports Cloudflare R2 / Backblaze B2; private ACL, no overwrite, 1h signed URLs); without it, `FileSystemStorage` with `MEDIA_ROOT` overridable from env for a Render Persistent Disk mount (single instance only). Both modes import-verified.

**pytest-xdist** added to requirements: `pytest -n auto` parallelizes the suite on multi-core dev machines (no effect in single-core CI containers).

Suite: 342 tests + 30 subtests, all passing.

## Email verification + JS test harness and helper extraction

**Email verification (soft gate).** Signup stays friction-free: the user is signed in and tracing immediately, but Stripe checkout requires a verified email so throwaway-address tenants never reach the payment flow. Pieces:

- `users.User.email_verified_at` (+ `email_verified` property); migration `users/0002`, and `users/0003` grandfathers every pre-existing user as verified via `date_joined` so nobody already using the app gets nagged or blocked.
- `accounts/emails.py`: stateless signed tokens (`django.core.signing`, salt `accounts.email-verification`, 3-day `max_age`), no token table. `send_verification_email` builds the absolute link and sends via the configured `EMAIL_BACKEND` (console in dev).
- `accounts.VerifyEmailView` (`/accounts/verify-email/<token>/`, no login required since the link may open in a different browser; the token alone proves inbox control), `accounts.ResendVerificationView` (POST, login required), and `SignUpView` sends the email on registration.
- Gate: `billing.views._require_verified_email(request)` guards both checkout views (`EstimateCheckoutSessionCreateView`, `SubscriptionCheckoutSessionCreateView`) with a message + redirect to the billing overview. Nothing else is gated.
- Banner: `templates/base.html` shows a `.verify-banner` (with a resend form) to any signed-in unverified user; styles in `main.css`.
- Tests in `accounts/test_signup.py::EmailVerificationTests`: outbox on signup, link verifies, garbage and expired tokens rejected (expiry via mocking `MAX_AGE_SECONDS`), resend, checkout blocked until verified, banner lifecycle, unverified-by-default for fresh users.

**JS helpers extracted + tested.** `plans/static/plans/viewer-helpers.js` is a UMD module (browser: `window.LumberViewerHelpers`, node: `module.exports`) holding the viewer's pure functions: `formatFeetInches`, `parseTraceIds`, `projectPointToSegment`, `snapToOrtho(previous, pointer, toleranceRad)`, and two new pure cores, `nearestPointWithin(points, pointer, radius)` (endpoint snap) and `nearestSegmentProjection(geometries, pointer, maxDist)` (segment snap + one-click opening placement). `viewer.js` now aliases the pure functions and keeps only the canvas-facing collection loops; it loads after `viewer-helpers.js` in `viewer.html`. Tests: `js_tests/viewer-helpers.test.js` (14 cases on node's built-in runner; run with `node --test js_tests/viewer-helpers.test.js`; note this node version rejects a bare directory argument) and `plans/test_js_helpers.py` bridges them into pytest via subprocess (skipped when node is absent), so one suite gates both languages. Further splitting of viewer.js should follow the same namespace pattern one seam at a time; the remaining file is state-heavy and not worth a big-bang split.

Suite: 350 tests + 30 subtests (including the JS bridge), all passing.

## Optional material pricing (bid totals)

STACK-level gap closed: quantities can now roll up to dollars, while the app stays a pure material list until the account chooses to price anything. Pricing is per-account and never touches the shared catalog.

- **Model.** `catalog.MaterialPrice` (migration catalog/0014): account FK + material FK + `unit_cost` Decimal(10,2), unique per account+material, `updated_at`. Unit cost is per ordering unit: per stock piece for feet materials, per box for box materials, per each otherwise (model help_text documents this). Account-scoped by design: lumber pricing is a contractor's private edge, and pricing a global stock SKU must never mutate the shared row.
- **Rollup.** `_grouped_order_list` now selects `material_id`; `_attach_pricing(order_list, account)` bulk-reads the account's prices and sets `unit_cost` / `extended_cost` per row (None when unpriced), returning whether anything is priced. `_summary_totals` adds `material_cost`, `priced_rows`, and `has_pricing`.
- **Surfaces, all gated on `has_pricing` so unpriced accounts see zero cost UI:** estimate detail Order List (Unit Cost + Extended columns, dash for unpriced rows, tfoot Material total, header badge, and an unpriced prompt linking to the Library), print view (same columns + total), CSV export (Unit Cost + Extended Cost columns and a Material total row appended), and the live viewer summary bar (a "material cost" stat).
- **Entry points.** (1) `MaterialForm` gained an optional `unit_cost` field: initial loads the account's existing price, `_save_price()` upserts on save and a blank clears it; works on create and edit of owned materials. (2) `MaterialPriceUpdateView` (`library/materials/<pk>/price/`) is a price-only endpoint scoped to `visible_to(account)`, so global stock SKUs (all 161, including the seeded hangers) are priceable via the inline $ form now on every Library material card; a `tag-priced` chip shows the current price. Blank clears; negative and garbage rejected with messages.
- **Tests.** `estimating/test_pricing.py` (13): rollup with full/partial/no pricing, cross-account leak prevention, CSV column gating both ways, form upsert + clear + per-account scoping, price endpoint on global materials (catalog untouched), negative rejection, library display.

Suite: 363 tests + 30 subtests, all passing. Fresh-database migrate verified (161 materials, 53 assemblies, MaterialPrice table present).

## UX Roadmap: `docs/Lumber_Estimating_App_Recommendations.pdf`

An external recommendations doc (20 items + a flagship "Sketch Mode") was reviewed against current app state. Most of the list is already substantially built under different names; the gaps are real but narrower than the doc implies. Status and phased plan below. **Where this plan disagrees with the source doc, that's called out explicitly rather than silently dropped or silently adopted.**

### Already implemented (no new work - doc is describing existing behavior)

- **#1 Draw First, Configure Later** - semantic tools auto-load a default assembly (`Assembly.is_default`, see "Default assemblies"); drawing produces a takeoff before anyone opens a dropdown.
- **#2 Smart Defaults** - `JobSettings` parameterizes every calc; tool memory + `is_default` assemblies remove repeat configuration.
- **#4 Keyboard Shortcuts** - W/O/B/J/C/P/R/H tool shortcuts, Enter/Escape/Backspace/D/Page Up/Down (see "Start Takeoff + keyboard shortcuts", "Drawing assists").
- **#5 Persistent Draw Mode** - tools stay armed ("sticky") until explicitly deactivated; documented under "Tool" in Core Domain Concepts.
- **#7 Automatic Corners** - corner/T-intersection stud detection is fully built ("Corner & Wall-Intersection Studs").
- **#8 Live Material Totals** - the viewer's material list panel + summary bar refresh after every trace op ("Material List Output", "Live summary bar").
- **#18 Reduce Settings** - Project Templates + JobSettings already centralize configuration at project setup.
- **#12 Favorite Assemblies (partial)** - `ToolPreset` + tool memory + `is_default` cover the "don't re-pick the same assembly" goal, though nothing is labeled "favorite" yet (see Phase A).
- **#13 Apply to Similar (partial)** - "Bulk actions for linked groups" + canvas multi-select already do batched material/assembly/color updates across a selection (see Phase A for the auto-detect-matching-walls extension).
- **#6 Predict User Intent (partial, deliberately not fully automated)** - tool memory already re-arms the last-used settings per tool/variant. See "Where this plan deviates" below for why we're not building further automatic prediction on top of it.

### Phase A - near-term, moderate effort, high day-to-day value (shipped)

1. **Confidence indicators (#20).** ✅ Shipped.
2. **Right-click context menu (#10).** ✅ Shipped.
3. **Favorite assemblies, made explicit (#12).** ✅ Shipped.
4. **Apply to similar, generalized (#13).** ✅ Shipped (page-scoped, client-side).

See "Phase A implementation notes" below for what actually shipped and where it differs from the plan as written above.

### Phase B - medium-term, real UX rearchitecture (shipped)

5. **Live editing of trace geometry (#14, extends existing member-level editing).** ✅ Shipped.
6. **Automatic wall joining (#15).** ✅ Shipped.
7. **Floating quick-edit HUD (#9), scoped down.** ✅ Shipped.
8. **Visual estimate dashboard (#19).** ✅ Shipped.

See "Phase B implementation notes" below for what actually shipped and where it differs from the plan as written above.

### Phase C - long-term / high-risk, needs a validation spike before committing real build time

**#10 and #11 (both AI/model-dependent) are explicitly deferred - decided 2026-07-08.** Both need either a paid model/vision API subscription or new background-job infrastructure (or both) before there's anything to build. The owner's call: wait until the app has paying customers/revenue before taking on a recurring AI subscription cost, rather than speculatively building against an unvalidated feature. Revisit once the app is generating revenue. #9 and #12 don't have this dependency and remain open to pick up independently.

9. **Command palette (#17).** Lower risk than the others in this phase, but low ROI until the action surface (right-click menu, bulk tools, shortcuts) from Phase A/B exists to search over - sequence this last in Phase C, or bundle into Phase A if it turns out cheap with an existing library. No AI/subscription cost - not blocked by the deferral above.
10. **Automatic plan recognition (#11). DEFERRED - needs a paid vision/ML API or trained model, plus background-job infra that doesn't exist yet.** CV/ML wall-and-opening detection from a rasterized PDF/image. Real value (huge click reduction) but real risk: false positives directly corrupt a material list, which violates "the material list is the product" (Key Business Rules). Needs a spike against real customer plans to establish accuracy before any UI commitment, and should ship as a **suggestion layer the user must confirm per element**, never auto-committed traces.
11. **AI Assistant (#16). DEFERRED - same reason as #10.** "Intelligent framing suggestions" is not yet a scoped feature. Do not start building until there's a concrete, narrow use case (e.g. "flag a wall assembly that's inconsistent with its neighbors") rather than an open-ended assistant, and until the subscription-cost question above is revisited.
12. **Sketch Mode (flagship, reframed - see disagreement below).** ✅ Shipped. Re-scoped from "freehand sketch -> auto-framed assemblies" to "fast full-building layout on a calibrated page using straight-line/ortho-snap tools that are already built" (endpoint snap, ortho snap, one-click openings). A true uncalibrated freehand-to-BOM pipeline is a separate, much larger CV problem and was correctly not needed for this rename/rescope. See "Sketch Mode implementation notes" below.

### Where this plan deviates from the recommendations doc

- **Sketch Mode as described conflicts with a core business rule.** The doc's flagship feature is a rough freehand sketch that gets converted into framed assemblies automatically. Every quantity in this app is currently derived from a *calibrated, measured* trace (`PlanPage.scale_pixels_per_foot`) - that's what makes the material list trustworthy enough to hand to a lumber yard. An uncalibrated freehand sketch has no real-world scale, so "auto-converts to framed assemblies" would mean either (a) fabricated quantities presented with the same confidence as measured ones, which is dangerous, or (b) the user must still calibrate/scale it, at which point it's not meaningfully different from the fast-drawing tools that already exist (ortho snap, endpoint snap, one-click openings, tool memory). Recommendation: don't build "Sketch Mode" as scale-free freehand; instead invest further in making calibrated drawing faster (already the direction of "Drawing assists"), and reserve the "sketch" framing for a low-fidelity *massing/estimate-only* mode if a real customer need for that shows up.
- **#11 Automatic Plan Recognition and #16 AI Assistant are speculative, not scoped.** Both are listed in Phase C specifically because they need a validation spike (real accuracy numbers, a narrow use case) before they're plannable work at all - the doc treats them as roughly equal-weight bullets alongside things like keyboard shortcuts, but they're a different category of effort (ML/CV pipeline, ongoing model maintenance) with a much higher cost of being wrong (a hallucinated wall or phantom material line item is worse than no suggestion). Don't start either without a separate accuracy/scope discussion first.
- **#3 Simplified Toolbar - disagree with literally reducing the tool count.** The doc proposes collapsing to Wall/Opening/Floor/Roof/Beam/Notes/Measure. The app already has more precision than that (Wall/Opening/Beam each expand into subtype variants - exterior/interior-bearing/interior-non-bearing, window/door, flush/dropped - plus Joist/Column/Plate/Rim Board), and that precision is what drives correct default-assembly selection. Collapsing back to 7 generic tools would lose real calculation accuracy. Prefer: keep the current semantic toolbar, but make sure the *default* collapsed/mobile view surfaces only the highest-frequency tools (Wall, Opening, Beam, Joist) with the rest under an overflow, rather than removing capability. Note: a non-persisting **Measure** tool (`data-tool="measure"`, ortho/endpoint-snapped, shows a live dimension without creating a Trace) already exists in the toolbar - the doc's #3 gap here is narrower than it looks. **Notes/annotation** (a free-floating text callout on the plan, unrelated to material calculation) is the one genuinely missing primitive from the doc's list.
- **#6 Predict User Intent - disagree with building further automatic prediction beyond tool memory.** Tool memory (last-used settings/assembly/material per tool+variant) already solves "don't re-prompt for the same wall type." Adding a heuristic that *guesses* the next wall's type from geometry/context risks silently mis-assigning a wall assembly the user didn't choose, which is worse than asking once. Recommend leaving this as-is unless a specific repeated friction point shows up in practice.

## Phase A implementation notes

All four Phase A items from the roadmap above are built. Details and where the shipped version differs from the original plan text:

- **Confidence indicators.** No new model - computed client-side from data the viewer already has (`assembly_id`, `parent_wall_id`). A trace is "incomplete" if it has no assembly, or (opening-specific) if it has no `parent_wall`. Two surfaces: a small orange "!" badge (`fabric.Group` of a circle + text) drawn at the trace's first geometry point on the canvas (`updateIncompleteMarker`/`incompleteMarkers` map in `viewer.js`), and a warning line in the trace inspector (`#inspector-confidence-status`). Both stay in sync across create, single-trace auto-save, and batch update/delete paths. Deliberately *not* built: the page-strip-level "which pages have incomplete traces" rollup from the original plan text - that needs a per-project-page query the single-page confidence check doesn't, and wasn't worth the added backend cost for v1.
- **Right-click context menu.** `#trace-context-menu` (a plain `.dropdown-menu`, visibility toggled via inline `style.display`, not Bootstrap's JS dropdown) with **Duplicate**, **Select similar**, **Apply current tool setup** (hidden unless a tool is armed and matches every trace in the current selection), and **Delete** (relabels to "Delete linked (N)" for a multi-selection). Every action reuses an existing function (`duplicateTrace`, `selectSimilarTraces`, `applyCurrentToolSetupToLinkedTraces`, `deleteTrace`/`deleteLinkedTraces`) - no new endpoints. **Important gotcha fixed during build:** the menu must *not* be suppressed just because a tool is armed - tools stay armed between draws by design ("persistent draw mode"), so gating on `activeTool` would have made the menu nearly unreachable in normal use. It's gated on `pendingPoints.length > 0` instead (only an in-progress multi-point shape blocks it), the same distinction Escape already makes.
- **Favorite assemblies via ToolPreset.** `ToolPreset` gained `assembly` (FK, nullable, tool-type-validated via the existing `_resolve_assembly` helper) and `is_favorite` (bool; `Meta.ordering` now `['-is_favorite', 'name']`). This closes a real pre-existing gap, not just a UI addition: presets previously saved material + settings + color but silently **dropped the assembly**, so loading a preset never restored what assembly to use. `ToolPresetFavoriteToggleView` (`plans:preset-favorite`) is a simple POST toggle, account-scoped. Frontend: the preset `<select>` sorts favorites first with a ★ prefix (`Object.keys()` on numeric-string ids iterates in ascending numeric order regardless of insertion order in JS, so this needs an explicit sort, not just relying on the model's own ordering), plus a star button next to it.
- **Apply to similar / Select similar.** Fully client-side, page-scoped (matches by `traceToolType` + `traceMaterialId` across the canvas objects already loaded for the current page) - reuses `activateLinkedTraceIds()`, the same function the material-list-row-click and canvas-multi-select paths already use, so it lands in the same bulk-edit panel with the same batch endpoints. Exposed two ways: the context menu's "Select similar" item, and a "Select similar" button in the trace inspector panel (for discoverability without needing to know about right-click). Cross-page/project matching (the "or project" half of the original plan bullet) was not built - would need a new backend query since only the current page's traces are loaded into the canvas.
- **Verified live**, not just via pytest: spun up the dev server against the local Postgres instance, created an isolated throwaway account/project/plan page (cleaned up afterward, including its media files), and drove it with a headless-Chromium Playwright script (logged in, drew a wall with the raw Line tool, confirmed the incomplete badge and inspector warning appeared, right-clicked to confirm the context menu contents, assigned an assembly and confirmed the badge cleared, saved+favorited a preset and confirmed the assembly and star persisted). Full suite (`pytest`, 463 tests) and `node --check` on `viewer.js` both pass.
- **Pre-existing issue found, not fixed:** the repo has a second, newer batch of git-tracked Dropbox "conflicted copy" duplicate files (dated 2026-07-05 - `estimating/test_views (mprovost's conflicted copy 2026-07-05).py` and similar in `catalog/`, `estimating/`, `plans/`, `docs/`, `templates/`), distinct from the batch already cleaned up in "Repo hygiene" above. One of them has a stale, failing test unrelated to this work. Left alone since removing tracked files wasn't in scope for this pass - worth a dedicated cleanup.

## Phase B implementation notes

All four Phase B items are built. This phase touches `Trace.geometry` mutation, which was previously immutable after creation - the riskiest change of the two phases so far, and it surfaced a real concurrency bug (below).

- **Live editing of trace geometry.** `TraceUpdateView`/`_update_trace_from_payload` (`plans/views.py`) now accepts an optional `geometry` key for `line`/`polyline` traces only - `_validate_geometry_update()` requires the same point count as the existing trace (repositioning, not reshaping: no adding/removing vertices through this endpoint) and that every point has numeric `x`/`y`. On a successful geometry update, `_regenerate_sibling_walls(trace)` runs (the same helper already used when a wall is first drawn or deleted), so a dragged endpoint that newly meets or separates from a neighboring wall picks up/loses its corner-stud bonus correctly. Frontend: selecting a `line`/`polyline` trace shows a small draggable circle "grip" at each vertex (`enterVertexEditMode` in `viewer.js`); dragging rebuilds a dashed ghost preview live via `buildTraceObject()` (chosen over mutating the real Fabric `Line`/`Polyline`'s own `points`, which is fragile) and only PATCHes + rebuilds the real trace object once a grip is released (`object:modified`). Scoped to line/polyline only, matching the plan - area/opening/count vertex dragging is a future extension if needed.
- **Automatic wall joining ("Fix nearby wall joints").** A button in the material list panel's "Filters & actions" menu scans every wall endpoint on the current page and pairs up any two (from different traces) within `JUNCTION_TOLERANCE_FT` (0.5 ft, matching `wall_junctions.py`'s existing corner/T-detection tolerance) but not already exactly coincident, then snaps each pair to its midpoint. This is a **purely cosmetic** fix - the calc engine already counts endpoints within that same tolerance as touching, so closing the visual gap doesn't change any existing quantity; the button's tooltip says so explicitly to avoid implying otherwise. **Bug found and fixed during browser verification:** the first version fired one PATCH request per affected trace via `Promise.all` (concurrently). Two walls that each land on the other's `_regenerate_sibling_walls` pass triggered a genuine **Postgres deadlock** (`OperationalError: deadlock detected`, each transaction locking its own `LineItem` rows and then reaching for the other's, in opposite order). Fixed by chaining the PATCH requests sequentially instead - each transaction fully commits before the next starts, so lock order can never cross. This is a real, unit-testable lesson: any future feature that batches trace geometry/assembly updates across *multiple HTTP requests* (as opposed to one request looping server-side, like `TraceBatchUpdateView` already does safely) needs to go sequentially, not concurrently, whenever the updates can touch each other's siblings.
- **Floating quick-edit HUD.** A small canvas-anchored card (`#trace-quick-hud`, built and positioned entirely in `viewer.js`, styled in `main.css`) appears next to a selected trace showing Material plus up to two tool-specific fields (stud spacing + wall height for line/open-polyline, spacing for area; nothing extra for opening/count/closed-polyline, matching the "handful of hottest fields" scoping decision). It has **no persistence logic of its own** - editing a HUD field writes the value into the corresponding real sidebar inspector field (`#inspector-material-select`, `#inspector-stud-spacing-input`, etc.) and dispatches a native `input`/`change` event on it, so the existing auto-save debounce (`scheduleSelectedTracePersist`) is the only code path that ever saves anything. Position is computed by converting the trace's first geometry point (stored in original image-pixel space) to screen coordinates via the canvas element's current `getBoundingClientRect()` width ÷ its raster width - the same ratio approach already used for the live drawing-length readout, so it stays correct across zoom levels without hardcoding a scale factor.
- **Clickable category header cards.** Each category header in the material list panel (`templates/estimating/_material_summary.html`) is now a clickable row with a piece-count/line-count badge (computed client-side in `annotateMaterialCategoryHeaders()` by summing each row's `data-quantity` - no backend change) - clicking it gathers every `data-trace-ids` value from that category's rows and calls the same `activateLinkedTraceIds()` used by a single material row, landing in the existing bulk-edit panel (or the single-trace inspector, if the category happens to link to just one trace). Deliberately scoped down from a standalone "dashboard" screen to an extension of the existing panel, per the roadmap's own disagreement note above.
- **Verified live**, same method as Phase A: dev server against local Postgres, an isolated throwaway account/project/plan page (cleaned up after, including media files), driven with headless-Chromium Playwright. This pass is what caught the deadlock - the bug never showed up in `pytest` (each test hits the endpoint once) or in casual manual testing (a human wouldn't naturally fire two saves at the exact same instant), only in a scripted test that intentionally exercised the "several nearby gaps at once" case. Confirms the value of the browser-verification step beyond what the test suite alone would have caught.
- **#19 Visual Estimate Dashboard - scoped down from "dashboard" to an in-viewer feature.** The doc implies a standalone dashboard screen; Phase B instead extends the existing Estimate Detail / material list panel (which already groups by category and already links back to source traces) rather than building a second, parallel summary surface that could drift out of sync with the real Order List.

## Repo hygiene: conflicted-copy cleanup (closed out)

The batch of git-tracked Dropbox "conflicted copy" duplicate files flagged in the Phase A notes (dated 2026-07-05, in `catalog/`, `estimating/`, `plans/`, `docs/`, `templates/`) has been removed via `git rm` - 14 files, none referenced anywhere else in the codebase (module names with spaces/parentheses/apostrophes aren't importable Python/valid statics to begin with, so they were pure dead weight, only ever getting picked up by pytest's test-file discovery). This is what was silently inflating past suite counts and causing the one recurring stray failure (`estimating/test_views (...).py::EstimateCsvExportViewTests`). Suite is now 374 tests + 30 subtests, all passing, with no known-failing tests left unaddressed.

## Sketch Mode implementation notes

Shipped as a toggle, not a new drawing primitive - the entire feature is a thin behavioral layer over tools that already existed (semantic exterior-wall polyline tool, endpoint/ortho snap, tool memory, live length readout), matching the roadmap's rescope decision.

- **Toggle button** (`#sketch-mode-toggle`, magic-wand icon, `S` shortcut via the existing `SHORTCUT_SINGLES` map) sits at the front of the viewer toolbar. Clicking it when off arms the Exterior Wall semantic tool (`[data-semantic="wall"][data-wall-subtype="exterior"]`, which is the existing `polyline` tool under the hood) and sets `sketchModeActive = true`; clicking it again finishes any in-progress shape and returns to the Hand tool.
- **Forced ortho-snap.** `applyDrawingSnaps()`'s existing Shift-held escape hatch (`domEvent.shiftKey` skips all snapping) now also checks `!sketchModeActive`, so square-angle snapping can't be bypassed while sketching - the whole point of the mode is a clean, fast outline.
- **Cumulative length readout.** The live drawing-length chip (`#draw-measure-readout`) normally shows only the in-progress segment; in Sketch Mode it shows a running "Total:" sum of every committed segment plus the live one (`cumulativePendingLengthPx()`), so a full perimeter walk gives a continuously-updating footage total instead of restarting at each corner.
- **Two real bugs found and fixed during browser verification** (both are the kind of interaction-order/state-lifecycle bugs that only show up when you actually drive the UI, not in unit tests):
  1. `activateTool()` clears the `.active` CSS class off every toolbar button as part of its own bookkeeping (including buttons outside its own semantic/raw-tool set, since the sketch toggle lives in the same `#tool-toolbar`). The toggle handler was originally adding `.active` *before* calling the wall button's `.click()`, which immediately wiped it. Fixed by re-applying `.active` *after* the click.
  2. The bigger one: this app auto-selects a trace right after it's created (`createTraceFromSnapshot` calls `selectTraceObject`), which internally calls `activateTool(null)` to open the inspector. The first version of the Sketch Mode exit guard treated *any* non-`polyline` tool value - including `null` - as "the user left Sketch Mode," so finishing even a single wall run silently cancelled the whole session before the next click. Fixed by only treating a genuinely different, truthy tool value as an exit signal (`tool && tool !== 'polyline' && tool !== 'calibrate'`) - a `null` from auto-deselect no longer clears it, so Sketch Mode now persists correctly across multiple separate wall runs in one sitting until the user explicitly toggles it off (or picks a different tool, or hits Escape while idle).
- **Verified live**, same method as Phases A/B: isolated throwaway account/plan page, headless-Chromium Playwright script. Confirmed: toggle arms with the correct hint text and default assembly, deliberately-imprecise clicks (a few px off a right angle) snap to an exact 0°/90° corner in the persisted trace geometry, the cumulative readout shows the running total, and toggling off correctly clears state. `pytest` (374 tests + 30 subtests) and `node --check` both pass.

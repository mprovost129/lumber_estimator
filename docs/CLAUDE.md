...
### Phase B - medium-term, real UX rearchitecture

5. **Live editing of trace geometry (#14, extends existing member-level editing).** Wall *member* overrides (studs/plates/headers) already support drag-to-edit (Phase 3/4). The open gap is at the trace level itself: dragging a wall's own endpoint to stretch/shorten it currently requires delete+redraw. Add draggable endpoint/vertex grips on selected line/polyline traces that PATCH geometry and trigger the same recalculation path `TraceUpdateView` already runs.
6. **Automatic wall joining (#15).** Endpoint snap (already built) handles the drawing-time case. Add a post-hoc "snap nearby endpoints together" cleanup action - when two wall endpoints are within tolerance but not exactly coincident (e.g. imported/legacy traces), offer a one-click merge that moves them to a shared point so junction detection (#7) fires correctly. Should reuse `wall_junctions.py`'s existing tolerance constant.
7. **Floating quick-edit HUD (#9), scoped down.** Rather than replacing the inspector sidebar, add a small canvas-anchored popover for the handful of hottest fields (material, stud spacing, wall height) that appears next to a selected trace. The full inspector panel stays for everything else (openings, overrides, parent-wall assignment) - see disagreement note below.
8. **Visual estimate dashboard (#19).** Turn the existing "Jump to source" / row-highlight behavior into the primary navigation surface: a card-per-category summary (mirrors the Order List's category grouping) where clicking a card highlights every contributing trace on the current page. Builds on data `_grouped_order_list()` already produces.
